...
- (NSString*) browseXMLFile 
{
	...
	[oPanel setTitle:@"CaveWriting - Select a Story XML File to Load"];

    [oPanel setAllowsMultipleSelection:NO];
    result = [oPanel runModalForDirectory:[NSString stringWithCString:workdir] file:nil types:fileTypes];
...
void CustomApplicationMain (int argc, char** argv)
{
	...
	//[[NSApp mainWindow] makeKeyAndOrderFront:0];
...
- (void) applicationDidFinishLaunching: (NSNotification *) note
{
    ...
    if (gFinderLaunch) {
		NSString *file = [self browseXMLFile];

		/* Hand off to main application code */
		if (file) {
			const char* blah = [file cString];
			gArgv[gArgc - 1] = strdup(blah);
		} else {
			gArgc -= 2;
		}
	}
...
int main (int argc, char **argv)
{
	...
	//IK Add params
	if (gFinderLaunch) {
		gArgv = (char**) malloc (sizeof(*gArgv) * (gArgc+3));
	} else {
