...
bool
ConfigMap::readFile(const std::string &filename)
{
  std::string instr;
  ...
  while (instr.size()) {
    ...
    **/
    ...
    while ((slash != nameval.npos) && (nameval[slash+1] != '\\') && (endline != nameval.npos)) {
    ...
    if (nameval[0] != '#') {
      ...
      if (doubleslash >= 0) {
	nameval = nameval.substr(0,doubleslash) + nameval.substr(doubleslash + 1);
      }
...
bool
ConfigMap::containsKey(const std::string &keyString)
{
  return _map.containsKey(keyString);
}

std::string
ConfigMap::getValue(const std::string &keyString)
{
...
ConfigMap::ConfigMap(int argc, char **argv, Log *log, bool exitOnUnrecognizedArgument)
{
  _log = log;
...
void
ConfigMap::printArgumentHelpAndExit(const std::string &programName)
{
